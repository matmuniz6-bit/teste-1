...
def ns_criticality_day(req: NavierStokesDayRequest):
    ...
    try:
        ...
        def _apply_probe(position_col: str, prefix: str):
            ...
            turn = (p - prev).abs()
            # Linear long/short exposure to the asset's simple return.
            # This fixes the previous inverse-price interpretation e^{-r}-1 for shorts.
            gross_simple = p * frame["simple_ret"]
            net = (1.0 + gross_simple) * (1.0 - cost_rate * turn) - 1.0
        ...
        def _probe_cost_sensitivity(prefix: str):
            g = day.dropna(subset=[f"{prefix}_gross_ret", f"{prefix}_turnover"])
            gross = g[f"{prefix}_gross_ret"]
            turn = g[f"{prefix}_turnover"]
            out = {}
            for bps in (0, 2, 5, 10, 15, 20):
                cr = bps / 10_000.0
                step = (1.0 + gross) * (1.0 - cr * turn) - 1.0
        ...
        return {
            ...
            "v3_event_momentum_3h": {
                ...
                "return_accounting": "linear long/short exposure to asset simple return, then multiplicative turnover costs",
                "cost_bps_per_turnover": req.illustrative_cost_bps_per_turnover,
